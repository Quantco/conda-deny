...
fn _get_environment_names(
    ...
    environment_names
        .into_iter()
        .filter(|env_name| !excluded_environments.contains(env_name))
        .collect()
}
...
pub fn get_package_records_for_pixi_lock(
    ...
    exclude_environment_spec: Vec<String>,
    ...
    if environment_spec.is_empty() {
        environment_spec = _get_environment_names(pixi_lock_path, exclude_environment_spec);
    }
...
mod tests {
    ...
    fn test_pixi_lock_read_out() {
        let environment_names = _get_environment_names(
            Path::new("tests/test_pixi_lock_files/valid1_pixi.lock"),
            vec![],
        );
    ...
    fn test_get_packages_for_pixi_lock() {
        ...
        let package_records = get_package_records_for_pixi_lock(Some(path), vec![], vec![], vec![]);
        assert_eq!(package_records.unwrap().len(), 758);

        let package_records =
            get_package_records_for_pixi_lock(Some(path), vec!["lint".to_string()], vec![], vec![]);
